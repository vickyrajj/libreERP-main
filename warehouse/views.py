...
def genAllInvoice(response,contract,details,request):
        ...
        if details.data!=None:
            detailsData = json.loads(detailsData)
            print detailsData,'aaaaaaaaaaaaaaaaaa'
        # if detailsData!="null" or details.data!=None:
            for i in detailsData:
                ...
                if 'space' in i:
                    ...
                    space =''
                row3 += [[dsc,code,space, rate,qty , '',amount]]
                bamount += amount
        total = bamount
        if(str(contract.company.gst[0:2])=='29'):
            ...
            cgst = 9
            igst = 0
            gsttotal = details.totalTax/2
            cgsttotal = details.totalTax/2
            igsttotal = 0
...
class DownloadMonthlyInvoice(APIView):
    ...
    def get(self , request , format = None):
        response = HttpResponse(content_type='application/pdf')
        print request.GET['contract'],'aaaaaaaaaaaaaaaaaaaaaaaaa'
        o = Contract.objects.get(id = request.GET['contract'])
        invoiceobj=Invoice.objects.get(contract=request.GET['contract'],pk=int(request.GET['inoicePk']))
